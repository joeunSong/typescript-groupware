...
const OrganzationLayout = (props: any) => {
  const { organization, handleDepartmentClick, handleUserClick } = props;

  // * Panel Header 템플렛
  const panelHeaderTemplate = (options: any, type: any, item: any) => {
    console.log('item', item);
    return (
      <div className={`flex w-full items-center py-1 hover:bg-gray-100 cursor-pointer `}>
        {/* 본인에게 자식이 더 있다면 아이콘 */}
        <div
        ...
        <div className={`flex w-full items-center gap-1 `} onClick={() => handleDepartmentClick(item)}>
          ...
          <span className='noto-sans-kr text-base'>{item?.label}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className='flex w-full overflow-hidden'>
      ...
      {_.map(organization, (company: any, companyIndex: number) => {
        return (
          <div className='flex flex-col w-full' key={companyIndex}>
            {/* 회사 */}
            <Panel
              className='w-full'
            ...
            >
              ...
              {_.map(company?.departments, (department: any, departmentIndex: number) => {
                return (
                  <div className={`flex flex-col w-full ml-3`} key={departmentIndex}>
                    <Panel
                      className='w-full'
                    ...
                    >
                      ...
                      {_.map(department?.users, (user: any, userIndex: number) => {
                        return (
                          <div className='flex w-full items-center py-1 hover:bg-gray-100' key={userIndex}>
                            <div className='flex w-full items-center pl-10 cursor-pointer' onClick={() => handleUserClick(department, user)}>
                              ...
                              <span className='noto-sans text-base'>{user?.label}</span>
                            </div>
                          </div>
                        );
                      })}
